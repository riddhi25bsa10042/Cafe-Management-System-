...
def create_item():
    ...
    try:
        ...
        if category not in ["beverage", "food"]:
            print("❌ Category must be 'beverage' or 'food'!")
            return
...
def take_order():
    ...
    if not name:
        name = "Guest"
        print("⚠️  Using 'Guest' as customer name")
    ...
    while True:
        ...
        try:
            ...
            if qty <= 0:
                ...
                continue
            if qty > item["stock"]:
                print(f"❌ Only {item['stock']} items available!")
                continue
...
def simulate_sales():
    ...
    for day in range(7):
        ...
        bar_length = max(1, daily_total // 200)
        bar = "█" * bar_length
        print(f"Day {day+1}: {bar} Rs {daily_total}")
